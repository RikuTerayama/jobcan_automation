...
### **AutoFill キュー運用メモ（UX・問い合わせ用）**
- **タブを閉じてもキャンセルにはならない**。進捗表示が止まるだけで、サーバ上の処理は続く場合がある。止めたい場合は待機中に UI の「キャンセル」を押すよう案内する。
- **待機中（queued）はキャンセル可能**（`POST /cancel/<job_id>`）。**実行中（running）は現状キャンセル不可**。
- キューはインメモリのため **再起動でキューは消える**。待機中ジョブは復元されない。
