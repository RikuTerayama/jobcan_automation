...
- `MAX_QUEUE_SIZE=5`: recommended upper bound for the in-memory waiting queue.
- `WEB_CONCURRENCY=1` and `WEB_THREADS=1`: recommended to avoid Chrome memory
  pressure on the free plan.
- `MAX_FILE_SIZE_MB=10`: keeps uploaded Excel files small enough for the free
  instance.
...
**処理方式は機能により異なります。**
- **画像/PDF/CSV/SEO 等のツール**: ブラウザ内で処理。ファイルはサーバーに送信されません
- **Jobcan AutoFill**: サーバー側で一時受領して処理し、処理後に削除します。継続保存はしません
...
- `MAX_QUEUE_SIZE`: 待機キューの最大長（既定 5）。超過時は 503 QUEUE_FULL
...
- **複数人が同時にアップロード可能。実行は直列（1件ずつ）**。2件目以降は 503 で弾かず、**待機キュー（インメモリ FIFO）に積み、`job_id` を返す**（HTTP 202 Accepted）。順番が来たら自動で開始。
- UI は「待機中 → 実行中 → 完了/失敗」をポーリングで表示。
- **/upload で 503 を返すのは次のみ**: (1) キュー満杯（`error_code: "QUEUE_FULL"`）、(2) メモリガード超過。**「同時処理数上限」で 503 は返さず、必ず queued で job_id を返す。**
- 待機キューはインメモリのため **サーバー再起動でキューは消える**。`QUEUED_MAX_WAIT_SEC`（既定 1800＝30分）超過で待機ジョブは timeout 扱い。`MAX_QUEUE_SIZE`（既定 5）でキュー長上限。

**AutoFill キュー利用上の注意（UI と同トーン）:**
- **タブを閉じてもキャンセルにはなりません**。進捗表示は止まりますが、サーバ上の処理は続く場合があります。止めたい場合は待機中に「キャンセル」を押してください。
- **待機中（queued）はキャンセル可能**です（`POST /cancel/<job_id>`）。実行中（running）は現状キャンセル不可です。
- キューはインメモリのため **アプリ再起動でキューは消えます**。待機中ジョブは再起動後は復元されません。
